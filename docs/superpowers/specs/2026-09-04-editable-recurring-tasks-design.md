...
| `clientId`, `clientName`, `taskName`, `taskDescription`, `serviceId`, `assignedMembers`, `recurrence`, or `priority` is passed as an *explicit* `null` (distinct from omitted — GraphQL argument omission arrives as JS `undefined`, but these fields are nullable in the schema so a caller can also send `null` explicitly) | Rejected with a clear error (`"<field> cannot be cleared — omit it to leave it unchanged, or provide a real value."`) before any validators run. These fields are non-null/required on the `RecurringTask` type or load-bearing for the scheduler (`recurrence` in particular — Firebase RTDB's `.update()` treats a `null` value as a delete-the-key instruction, and a missing `recurrence` sends `computeNextRun` into an infinite loop in `runDueRecurringTasks`, hanging the whole API server). `departmentId` and `assignedUsers` are intentionally exempt — they're nullable/list fields on the schema and clearing them via explicit `null` is legitimate, matching `editTask`'s existing convention. `runDueRecurringTasks` also independently skips (and logs) any stored template whose `recurrence` isn't a recognized value, as defense-in-depth against bad data from any source. |
