...
> **Corrected after this plan shipped:** this section originally claimed the frontend never writes to Firebase directly. That was wrong — the final whole-branch review found `src/lib/chat.ts`, `notes.ts`, `calendar.ts`, `notifications.ts`, `inquiries.ts`, `quickLinks.ts`, `directory.ts`, `statusColors.ts`, and `entityColors.ts` all write directly from the anonymous browser client, to `groups/*`, `meetingNotes`, `calendarEntries`, `notifications`, `statusColors`, `clientColors`, and `serviceColors`. The first published rules version (`.write: false` at root with no exceptions) broke every one of those features live in production. `database.rules.json` now grants `.write: true` back to exactly those paths, alongside the root lockdown. See the git history on `database.rules.json` for the fix. Any future read-scoping work (Piece 2) needs to account for these paths too, not just the backend-owned collections below.

The backend's Firebase usage is fully migrated to the Admin SDK — see the Components section below. The frontend's read path (`src/lib/realtime.ts`) is unaffected by this plan and stays read-only via `.read: true`; the frontend's WRITE paths, corrected above, are the one part of this plan's original "frontend is untouched" premise that didn't hold.
